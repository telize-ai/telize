...
EXAMPLE = Path(__file__).resolve().parents[1] / "examples" / "env_config.yaml"


def test_api_url_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OLLAMA_HOST", "192.168.1.50")
    spec = load_spec(EXAMPLE)
    assert spec.models["default"].api_url == "http://192.168.1.50:11434"


def test_missing_env_var_in_models(tmp_path: Path) -> None:
        ...
        dedent("""
            ...
            models:
              default:
                provider: openai
                model: m
                api_url: http://{{ env.TELIZE_MISSING_VAR_XYZ }}:11434
            flows:
              main:
                steps:
                  - name: a
